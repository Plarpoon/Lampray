...
Lamp::Game::lampReturn Lamp::Game::BG3::preCleanUp()
{
    ...
    if (std::filesystem::exists(appPath) && std::filesystem::is_directory(appPath) && std::filesystem::exists(tempAppPath) && std::filesystem::is_directory(tempAppPath))
    {
    ...
    if (!skipMount)
    {
        ...
        Core::Base::lampLog::getInstance().log(
            BG3OverlayMods->create(std::filesystem::absolute(KeyInfo()["appDataPath"] + "/Mods"),
                                   absolute(ModsConfig), absolute(ModsWorkingDir))
                .returnReason,
